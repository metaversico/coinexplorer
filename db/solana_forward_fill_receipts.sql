...
-- Function to get forward-fill state for a given market
-- Returns JSON with: { "state": "first_run|pending|ready", "next_signature": "<sig>|null" }
CREATE OR REPLACE FUNCTION get_market_forward_fill_state(market_name TEXT)
RETURNS JSONB AS $$
DECLARE
    receipt_target_uri TEXT;
    rpc_call_result JSONB;
    result_array_length INTEGER;
    next_signature TEXT := NULL;
    has_pending_call BOOLEAN := FALSE;
BEGIN
    -- Find the most recent receipt for this market's forward-fill job
    SELECT r.target_uri INTO receipt_target_uri
    FROM receipts r
    WHERE r.origin_uri = 'solana-signature-forward-fill/' || market_name
      AND r.target_uri LIKE 'rpc_call/%'
    ORDER BY r.created_at DESC
    LIMIT 1;

    -- If no receipt found, this is the first forward-fill run
    IF receipt_target_uri IS NULL THEN
        RETURN jsonb_build_object('state', 'first_run', 'next_signature', NULL);
    END IF;
    ...
    IF rpc_call_result IS NULL THEN
        RETURN jsonb_build_object('state', 'pending', 'next_signature', NULL);
    END IF;
    ...
    IF result_array_length IS NULL OR result_array_length = 0 THEN
        RETURN jsonb_build_object('state', 'ready', 'next_signature', NULL);
    END IF;

    -- Extract the first signature from the result array
    next_signature := rpc_call_result -> 0 ->> 'signature';

    RETURN jsonb_build_object('state', 'ready', 'next_signature', next_signature);
END;
$$ LANGUAGE plpgsql;

-- Grant execute permission to jobruns user
GRANT EXECUTE ON FUNCTION get_market_forward_fill_state(TEXT) TO jobruns;
